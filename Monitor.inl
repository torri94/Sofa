...
Monitor<DataTypes>::Monitor()
    ...
    ,d_stiffness ( initData ( &d_stiffness, 1, "stiffness", "stiffness value") )
...
void Monitor<DataTypes>::initGnuplot ( const std::string path )
{
    if ( !this->getName().empty() )
    {
        if ( d_saveXToGnuplot.getValue() )
        {
            ...
            m_saveGnuplotX = new std::ofstream ( ( path + "_"+ std::to_string(d_ID.getValue()) + "_x.txt" ).c_str() );
            (*m_saveGnuplotX)   << "#ID "
                                << d_ID.getValue()
                                << std::endl;
            (*m_saveGnuplotX)   << "#Stiffness"
                                << d_stiffness.getValue()
                                << std::endl;
