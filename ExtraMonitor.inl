...
ExtraMonitor<DataTypes>::ExtraMonitor()
    ...
    , ID  (initData ( &ID, 1, "ID", "simulation identity" ) )
{
...
void ExtraMonitor<DataTypes>::initGnuplot ( const std::string path )
{
    if ( !this->getName().empty() )
    {
        if ( this->d_saveXToGnuplot.getValue() )
        {
            ...
            this->m_saveGnuplotX = new std::ofstream ( ( path + "_" + std::to_string(ID.getValue()) + "_" + "x.txt" ).c_str() );
            (*this->m_saveGnuplotX) << "# ID " << std::to_string(ID.getValue()) << std::endl;
...
void ExtraMonitor<DataTypes>::exportGnuplot ( Real time )
{
    ...
    if ( this->d_saveXToGnuplot.getValue() )
    {
        ( *this->m_saveGnuplotX ) << sysTimeStr1 << " ";
