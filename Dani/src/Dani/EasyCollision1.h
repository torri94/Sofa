        ...
        for (const sofa::helper::vector<sofa::core::collision::DetectionOutput>* contact : contacts) 
        {
            for (const sofa::core::collision::DetectionOutput& detection_output : *contact) 
            {
                if (a1 == 0)
                {
                        ...
                        char* outputData = "1";
                        //strcpy(outputData, "std::to_string(1)");    //definisco la stringa che voglio passarci
                        DWORD lenght;                               //DWORD dimensione pari al doppio di un word
                        DWORD sizeBuffer= sizeof(outputData);                         // ho provato a metterci uno. Non so bene quanto metterci

                        if(WriteFile(hComm, outputData, sizeBuffer, &lenght, NULL) ==0) std::cout << "errore write";
